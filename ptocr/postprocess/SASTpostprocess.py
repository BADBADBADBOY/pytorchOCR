...
class SASTPostProcess(object):
    ...
    def __init__(self, config):
        self.score_thresh = config['postprocess']['score_thresh']
        self.nms_thresh = config['postprocess']['nms_thresh']
        self.sample_pts_num = config['postprocess']['sample_pts_num']
        self.shrink_ratio_of_width = config['postprocess']['shrink_ratio_of_width']
        self.expand_scale = config['postprocess']['expand_scale']
        self.tcl_map_thresh = config['postprocess']['tcl_map_thresh']
...
#             self.is_python35 = True
    ...
    def sort_coord(self,coord):
        points = sorted(list(coord), key=lambda x: x[0])
...
#             if np.sum(xy_text_scores) / quad_areas[instance_idx - 1] < 0.15:
                continue
...
#             detected_poly = self.sort_coord(cv2.boxPoints(cv2.minAreaRect(detected_poly.astype(np.int))))
