...
class hswish1(nn.Module):
    def forward(self, x):
        out = x * (F.relu(x + 3, inplace=True)-F.relu(x - 3, inplace=True)) / 6
        return out
...
class hsigmoid1(nn.Module):
    def forward(self, x):
        out =(F.relu(x + 3, inplace=True)-F.relu(x - 3, inplace=True))/ 6
        return out
